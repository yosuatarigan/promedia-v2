...
class LatihanFisikService {
  ...
  // Get latihan fisik logs by noKode and date
  Stream<QuerySnapshot> getLatihanFisikByDate(String noKode, DateTime date) {
  ...
  // Get latihan fisik logs by noKode and date range
  Stream<QuerySnapshot> getLatihanFisikByDateRange(
    String noKode,
    DateTime startDate,
    DateTime endDate,
  ) {
  ...
  // Delete latihan fisik log
  Future<void> deleteLatihanFisikLog(String logId) async {
    try {
      await _firestore.collection('latihan_fisik_logs').doc(logId).delete();
    } catch (e) {
